...
<script type="text/javascript">
	function check() {
		if (document.enrollform.title.value == "" || document.enrollform.title.value == null) {
			alert("제목은 필수입니다.");
			document.enrollform.title.focus();
			return false;
		} else if (document.enrollform.content.value == "" || document.enrollform.content.value == null) {
			alert("내용은 필수입니다.");
			document.enrollform.content.focus();
			return false;
		} else if (document.enrollform.writer.value == "" || document.enrollform.writer.value == null) {
			alert("작성자는 필수입니다.");
...
.sub_btn_menu .btn_line {margin: 1.5px 3px 1.5px 0;padding: 0 17px;min-width: 50px;ackground: #fff}
...
.board table tbody tr th {background-color: whitesmoke;font-size: 15px;font-family: '굴림';}
